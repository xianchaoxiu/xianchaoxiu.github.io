---
layout: page
title: 黎曼流形稀疏优化
comments: false
share: false
---


### 简介

黎曼流形稀疏优化是指决策变量带有稀疏性特征且满足黎曼流形约束的一类优化问题。作为当前运筹优化领域的研究热点，黎曼流形稀疏优化不仅在图像处理、无线通信、故障诊断等场景中得到了广泛应用，而且更有力地推动了数学、计算机、自动化等多学科的交叉发展。本书配套的全部实验代码均已整理上传，读者可前往作者的 GitHub 主页获取。


### 目录

* 第一部分 &nbsp; 基础篇 <br>
  - 第 1 章 &nbsp;  绪论 <br>
  - 第 2 章 &nbsp;  基于稀疏主成分分析的特征选择 <br>
  - 第 3 章 &nbsp;  基于稀疏联邦主成分分析的异常检测 <br>
  - 第 4 章 &nbsp;  基于稀疏正交非负矩阵分解的故障诊断 <br> 
  - 第 5 章 &nbsp;  基于稀疏张量相关分析的多视角学习 <br>
  - 第 6 章 &nbsp;  基于稀疏低秩对比学习的特征选择 <br><br>

* 第二部分 &nbsp; 进阶篇 <br>
  - 第 7 章 &nbsp;  基于深度张量低秩表示的图像去噪 <br>
  - 第 8 章 &nbsp;  基于深度自适应低秩稀疏的目标检测 <br>
  - 第 9 章 &nbsp;  基于注意力深度支持矩阵机的图像分类 <br>
  - 第 10 章 &nbsp;   基于自适应稀疏的大语言模型剪枝 <br><br>


* 第三部分 &nbsp; 拓展篇 <br>
  - 第 11 章 基于深度展开的图像反问题求解方法 <br>
  - 第 12 章 基于大语言模型的运筹优化求解方法 <br>

### 购买
TBD

### 反馈
* <a href="../OPT/黎曼流形稀疏优化.pdf" class="textlink" target="_blank"> 作者版本（推荐iPad阅读） </a> <br>
* <a href="https://docs.qq.com/form/page/DTmtyY2JXUWVjSEJS" class="textlink" target="_blank">如果您发现任何不妥和错误之处, 请通过此链接反馈 </a> <br>


