---
layout: page
title: 黎曼流形稀疏优化
comments: false
share: false
---

### 简介

本书《黎曼流形稀疏优化: 理论、算法与拓展》即将由哈尔滨工业大学出版社正式出版. 不仅涵盖黎曼流形稀疏优化领域, 也包含若干稀疏优化、深度学习与大模型的内容. 应用场景广泛，涉及图像特征选择、物联网异常检测、工业故障诊断、红外小目标检测、大模型剪枝等多个领域. 配套的全部实验代码均已整理上传, 读者可前往作者 GitHub 主页获取. 

### 目录

* 第一部分 &nbsp; 基础篇 <br>
  - 第 1 章 &nbsp;  绪论<br>
  - 第 2 章 &nbsp;  基于稀疏主成分分析的特征选择<br>
  - 第 3 章 &nbsp;  基于稀疏联邦主成分分析的异常检测<br>
  - 第 4 章 &nbsp;  基于稀疏正交非负矩阵分解的故障检测<br> 
  - 第 5 章 &nbsp;  基于稀疏张量相关分析的多视角学习<br>
  - 第 6 章 &nbsp;  基于稀疏低秩对比学习的特征选择<br>

* 第二部分 &nbsp; 进阶篇<br>
  - 第 7 章 &nbsp;  基于深度张量低秩表示的图像去噪<br>
  - 第 8 章 &nbsp;  基于深度自适应低秩稀疏的目标检测<br>
  - 第 9 章 &nbsp;  基于注意力深度支持矩阵机的图像分类<br>
  - 第 10 章 &nbsp;   基于自适应稀疏的大语言模型剪枝<br>


* 第三部分 &nbsp; 拓展篇<br>
  - 第 11 章 基于深度展开的图像反问题求解方法<br>
  - 第 12 章 基于大语言模型的运筹优化求解方法<br>


### 反馈
* <a href="../OPT/黎曼流形稀疏优化.pdf" class="textlink" target="_blank"> 下载作者版本 </a> <br>
* <a href="https://docs.qq.com/form/page/DTmtyY2JXUWVjSEJS" class="textlink" target="_blank">如果您发现任何不妥和错误之处，麻烦通过此链接反馈 </a> <br>


